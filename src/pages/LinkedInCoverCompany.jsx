...
async function drawCover(canvas) {
  ...
  // ── Logo (large, centered-left) ──
  const logoSVG = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 200 44" width="320" height="70">
  ...
  await new Promise((resolve) => {
    const img = new Image();
    img.onload = () => {
      ctx.drawImage(img, PAD + 10, 44, 320, 70);
  ...
  // ── Company headline ──
  ctx.fillStyle = "#FFFFFF";
  ctx.font = "800 58px 'Open Sans'";
  ctx.fillText("DISTINCT MARK CO.", PAD + 10, 215);

  // Orange underline
  ctx.fillStyle = "#E8832A";
  ctx.fillRect(PAD + 10, 230, 90, 4);

  // ── Tagline ──
  ctx.fillStyle = "#E8832A";
  ctx.font = "700 22px 'Open Sans'";
  ctx.fillText("BUILDING EXCELLENCE", PAD + 10, 278);

  // ── Sub-tagline ──
  ctx.fillStyle = "rgba(255,255,255,0.50)";
  ctx.font = "400 17px 'Open Sans'";
  ctx.fillText("Multi-disciplinary group · Riyadh, Saudi Arabia · CR No. 7004765322", PAD + 10, 322);
  ...
  ctx.fillText("www.distinctmark.net", W - PAD, H - 22);
  ctx.textAlign = "left";
}

export default function LinkedInCoverCompany() {
  ...
  const downloadJPG = async () => {
    ...
    link.download = "DistinctMark_LinkedIn_Cover_Company.jpg";
  ...
  return (
    <div className="min-h-screen bg-gray-200 py-10 px-4">
      <div className="max-w-5xl mx-auto mb-6">
        <h1 className="text-xl font-bold text-gray-700 mb-1">LinkedIn Cover — Company Page</h1>
